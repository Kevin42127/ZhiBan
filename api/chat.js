...
export default async function handler(req, res) {
  try {
    ...
    if (language === 'zh-TW') {
      ...
      if (isFirstMessage) {
        systemPromptContent = '你是智伴（ZhiBan），一個智能陪伴聊天助理。這是與用戶的首次對話，請簡單介紹自己，例如「我是智伴，你的智能陪伴聊天助理，很高興為你服務！有什麼我可以幫你的嗎？」。重要：請使用繁體中文（台灣）回應，絕對不要使用簡體中文。之後以自然、口語化的方式回應，用親切友善的語氣，就像朋友之間的對話一樣。回應要流暢自然，可以使用適當的標點符號和段落來讓內容更易讀。可以使用基本的項目符號（- 或 •）來組織列表內容，但不要使用 Markdown 格式、粗體、斜體或其他格式化符號。每個項目符號後要有空格，例如：「- 項目一」或「• 項目一」。';
      } else {
        systemPromptContent = '你是智伴（ZhiBan），一個智能陪伴聊天助理。重要：請使用繁體中文（台灣）回應，絕對不要使用簡體中文。以自然、口語化的方式回應，用親切友善的語氣，就像朋友之間的對話一樣。不需要明確提到身份，保持智伴的風格即可。回應要流暢自然，可以使用適當的標點符號和段落來讓內容更易讀。可以使用基本的項目符號（- 或 •）來組織列表內容，但不要使用 Markdown 格式、粗體、斜體或其他格式化符號。每個項目符號後要有空格，例如：「- 項目一」或「• 項目一」。';
      }
    } else {
      const isFirstMessage = !history || history.length === 0;
      if (isFirstMessage) {
